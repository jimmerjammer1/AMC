...
function AMCTracker() {
  ...
  const initializeData = async () => {
    try {
      ...
      const [appraisersRes, amcsRes] = await Promise.all([
        getFromLocalStorage('appraisers').catch(() => null),
        getFromLocalStorage('amcs').catch(() => null)
      ]);
      ...
      if (appraisersRes?.value && amcsRes?.value) {
        ...
        // Save to storage
        await saveToLocalStorage('appraisers', JSON.stringify(defaultAppraisers));
        await saveToLocalStorage('amcs', JSON.stringify(defaultAMCs));
      }
    } catch (error) {
  ...
  const saveAppraisers = async (newAppraisers) => {
    try {
      await saveToLocalStorage('appraisers', JSON.stringify(newAppraisers));
  ...
  const saveAMCs = async (newAMCs) => {
    try {
      await saveToLocalStorage('amcs', JSON.stringify(newAMCs));
      setAmcs(newAMCs);
    } catch (error) {
